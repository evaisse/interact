...
Dart defaults apply: two-space indentation, trailing commas on multiline collections, and `final` for values that never reassign. Follow the Conventional Lints enforced by `package:lint`; prefer explicit imports and null-aware operators to guard terminal interactions. Public APIs exposed via `lib/interact.dart` should use PascalCase classes and camelCase methods; files should mirror class names (`progress.dart`, `multi_spinner.dart`). Run the formatter before pushing to ensure consistent whitespace and ordering.
